...
- (void)viewDidLoad {
    ...
    tab1 = [[UIView alloc] initWithFrame:self.view.frame];
    tab2 = [[UIView alloc] initWithFrame:self.view.frame];
    tab3 = [[UIView alloc] initWithFrame:self.view.frame];
    tab2.hidden=YES;
    tab3.hidden=YES;
    ...
    tableWidth = [UIScreen mainScreen].applicationFrame.size.width-55;
    ...
    UIBarButtonItem *left = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"about"] style:UIBarButtonItemStyleBordered target:self action:@selector(doMenu:)];
    left.tintColor=[UIColor whiteColor];
    self.navigationItem.leftBarButtonItem =left;
    
    NSArray *segmentedData = [[NSArray alloc]initWithObjects:@"模拟器",@"食谱",@"食材",nil];
    UISegmentedControl *segmentedControl = [[UISegmentedControl alloc]initWithItems:segmentedData];
    segmentedControl.frame = CGRectMake(10.0, 20.0,self.view.frame.size.width-20, 30.0);
    ...
    [tab1 addSubview:resultView];
    [tab1 addSubview:scroll];
    [HUD hide:YES];

    table1 = [[UITableView alloc] initWithFrame:self.view.frame];
    table1.delegate=self;
    table1.dataSource=self;
    table1.tag=1;
    table2 = [[UITableView alloc] initWithFrame:self.view.frame];
    ...
    if([db openDB]){
        list1 = [db getStarveByType:@"3"];
        [table1 reloadData];
        
        list2 = [db getStarveByType:@"4"];
        [table2 reloadData];
    }
    
    [tab2 addSubview:table1];
    [tab3 addSubview:table2];
}
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(nameLabel==nil){
        nameLabel = [[UILabel alloc] initWithFrame:CGRectMake(50, 4, tableWidth, 22)];
    ...
    if(tagLabel==nil){
     ...
     if(tableView.tag==1){
         DataBean *bean = [list1 objectAtIndex:indexPath.row];
         if(bean){
             ...
             if(![Common isEmptyString:bean.content]){
                 CGSize size = [bean.content sizeWithFont:[UIFont fontWithName:@"Arial" size:12.0f] constrainedToSize:CGSizeMake(tableWidth, 1000) lineBreakMode:NSLineBreakByWordWrapping];
                 contentLabel.frame=CGRectMake(50, 30, tableWidth, size.height);
                 contentLabel.text = bean.content;
                 image.frame = CGRectMake(4, (size.height+10)/2, 40, 40);
                 tagLabel.frame=CGRectMake(50, size.height+35, tableWidth, 20);
                 tagLabel.text = [NSString stringWithFormat:@"%@,%@,%@",bean.p1,bean.p2,bean.p3];;
             }else{
        ...
        DataBean *bean = [list2 objectAtIndex:indexPath.row];
